...
export default function AttendancePage() {
  ...
  const [gpsStatus, setGpsStatus] = useState('idle'); // idle | detecting | found | failed
  ...
  const markAttendance = async (type) => {
    ...
    try {
      const res = await fetch('/api/attendance', {
        ...
        body: JSON.stringify({
          userId: user.id, date, type, locationId,
          customerIds, comments,
  ...
  const submitLeave = async (e) => {
    ...
    if (!leaveForm.fromDate || !leaveForm.toDate || !leaveForm.reason) {
      showToast('All leave fields are required', 'error'); return;
    }
    try {
  ...
  const displayHistory = isTeamView
    ? history.slice(0, 20)
    : history.filter(a => a.userId === user.id).slice(0, 20);
        ...
        <div className="fe-card">
          ...
          {todayRecord ? (
            <div style={{ background: 'var(--fe-teal-50)', border: '1px solid var(--fe-teal-100)', borderRadius: 10, padding: 16, textAlign: 'center' }}>
              ...
              <span className={`badge badge-${todayRecord.type}`} style={{ marginTop: 6 }}>
                {todayRecord.type === 'present' ? 'Present' : 'Leave'}
              </span>
          ...
          {displayHistory.length === 0 ? (
            ...
            <div style={{ maxHeight: 420, overflowY: 'auto' }}>
              {displayHistory.map(a => (
                <div key={a.id} style={{ padding: '10px 0', borderBottom: '0.5px solid var(--fe-gray-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    {isTeamView && <div style={{ fontWeight: 500, fontSize: 13 }}>{a.userName}</div>}
                    <div style={{ fontSize: 13, color: isTeamView ? 'var(--fe-gray-400)' : 'var(--fe-gray-800)', fontWeight: isTeamView ? 400 : 500 }}>{a.date}</div>
                    {a.locationName && <div style={{ fontSize: 12, color: 'var(--fe-gray-400)' }}><i className="ti ti-map-pin" style={{ fontSize: 11 }} /> {a.locationName}</div>}
                    {a.customerNames?.length > 0 && <div style={{ fontSize: 11, color: 'var(--fe-teal-600)', marginTop: 2 }}>{a.customerNames.slice(0, 2).join(', ')}{a.customerNames.length > 2 ? ` +${a.customerNames.length - 2}` : ''}</div>}
                    {a.comments && <div style={{ fontSize: 11, color: 'var(--fe-gray-400)', marginTop: 2, maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.comments}</div>}
                  </div>
